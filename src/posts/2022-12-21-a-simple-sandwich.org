...
#+begin_src haskell
-- A sandwich whose top and bottom slices are sourdough but consists of a single piece whose slices are whole grain
...
Now for the really interesting part. I left out one approach to error handling: eliminating all possible errors by encoding program flow into a type system. This only works in a statically-typed language, such as Haskell. Using the concepts of *type theory*, we can write code which ensures that the only way for it to run is if none of the errors occur. This safeguard check is done by the compiler, so as a programmer, we can be sure that if our code compiles, it will run without errors.
